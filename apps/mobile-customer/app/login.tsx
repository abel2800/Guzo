...
export default function LoginScreen() {
  ...
  useEffect(() => {
    (async () => {
      const last = await getLastEmail('customer');
      if (last) setEmail(last);
      const hasToken = !!(await tokenStorage.getAccessToken());
      const bio = await getBiometricEnabled('customer');
      const hw = await isBiometricHardwareAvailable();
      setShowBio(hasToken && bio && hw);
    })();
  }, []);
  ...
  async function onBiometric() {
    ...
    setBusy(true);
    try {
      if (!(await authenticateWithBiometrics('Unlock GUZO'))) return;
