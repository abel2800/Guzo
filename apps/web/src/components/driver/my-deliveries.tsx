          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            {o.status === 'ASSIGNED' && (
              <Button variant="outline" size="sm" onClick={() => setPickupOrder(o)}>
                <Camera className="h-4 w-4" /> Pickup proof
              </Button>
            )}
    ...
    <div className="space-y-6">
      ...
      {done.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm font-medium text-slate-400">Recently delivered</p>
          {done.slice(0, 5).map((o) => (
            <div key={o.id} className="dashboard-list-row flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2 text-sm">
              <CheckCircle2 className="h-4 w-4 text-emerald-500" />
              <span className="font-medium text-white">{o.orderNumber}</span>
              <span className="text-slate-400">{o.pickupAddress?.city} → {o.dropoffAddress?.city}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center">
          <Card className="w-full max-w-md">
            <CardContent className="space-y-4 p-6">
              <p className="font-semibold text-white">Mark delivery failed</p>
