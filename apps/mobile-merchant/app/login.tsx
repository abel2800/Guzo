...
export default function LoginScreen() {
  ...
  useEffect(() => {
    (async () => {
      const last = await getLastEmail('merchant');
      if (last) setEmail(last);
      const hasToken = !!(await tokenStorage.getAccessToken());
      setShowBio(hasToken && (await getBiometricEnabled('merchant')) && (await isBiometricHardwareAvailable()));
    })();
  }, []);
  ...
  async function onBiometric() {
    ...
    setError('');
    try {
      if (!(await authenticateWithBiometrics('Unlock GUZO Merchant'))) return;
          ...
          <GlassCard intensity={50} style={styles.formCard}>
            ...
            <Pressable onPress={() => router.push('/signup')} style={styles.registerLink}>
              <Text style={styles.registerText}>
                New merchant? <Text style={styles.registerBold}>Register business</Text>
              </Text>
            </Pressable>
          </GlassCard>
        </ScrollView>
